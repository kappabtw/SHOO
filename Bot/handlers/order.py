...
async def next_prev_order(callback: types.CallbackQuery):
	try:
		...
		order_id = await ASQL.execute(f"SELECT {t}(order_id) FROM Заказы WHERE order_id {direction} ?", (call_order_id))
		order_id = order_id[0][0]
		order_data = await ASQL.execute(f"SELECT * FROM Заказы WHERE order_id = ?", (order_id))
		order_data = order_data[0]
		order_model_id = order_data[0]
		order_user_id = order_data[2]
		order_user_name = order_data[3]
