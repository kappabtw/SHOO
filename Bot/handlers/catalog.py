...
async def cmd_catalog(msg: types.Message):
     await msg.answer(text = data.Message['brand']['def'], reply_markup = await catalogKeyboard.select_brands())
...
async def model_choice(callback: types.CallbackQuery):
    print(callback.data)
    await callback.message.edit_text(text = data.Message["model"]['def'], reply_markup=await catalogKeyboard.select_models(callback))    
...
async def callback_sales(callback:types.CallbackQuery):
    print(callback.data)
    await callback.message.edit_text(text = data.Message['brand']['sales'], reply_markup= await catalogKeyboard.select_brands_with_sales())
...
async def callback_new_items(callback:types.CallbackQuery):
    print(callback.data)
    await callback.message.edit_text(text = data.Message['brand']['new'], reply_markup= await catalogKeyboard.select_brands_new_items())
...
async def model_sales_choice(callback:types.CallbackQuery):
    print(callback.data)
    await callback.message.edit_text(text = data.Message['model']['sales'], reply_markup=await catalogKeyboard.select_models_with_sales(callback=callback))
...
async def model_new_items_choice(callback:types.CallbackQuery):
    print(callback.data)
    await callback.message.edit_text(text = data.Message['model']['new'], reply_markup=await catalogKeyboard.select_models_new_items(callback=callback))
...
async def callback_to_catalog(callback: types.CallbackQuery):
    print(callback.data)
    await callback.message.edit_text(text = data.Message['brand']['def'], reply_markup = await catalogKeyboard.select_brands())
...
async def callback_show_model_info(callback: types.CallbackQuery):
    ...
    current_index = 0
    txt = await show_model(list_id[current_index])  # Получаем подпись к изображению
    ...
    await callback.bot.send_photo(chat_id=callback.message.chat.id, photo=photo_id, caption=txt, parse_mode=ParseMode.MARKDOWN,
...
async def callback_prev_next(callback: types.CallbackQuery):
    ...
    if current_index < 0:
        ...
        current_index = 0
    txt = await show_model(list_id[current_index])
    photo_from_db = await ASQL.execute("SELECT Фото FROM Кроссовки WHERE id = ?", (list_id[current_index]))
    photo_id = photo_from_db[0][0]
